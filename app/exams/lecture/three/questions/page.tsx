...
export default async function LectureExamThreeQuestions() {
  const supabase = await createClient();

  // Fetch questions for lecture exam 3
  const { data: allQuestions, error } = await supabase.from("exam_questions").select("*").eq("exam_type", "lecture").eq("exam_number", "3");
  ...
  return <ExamQuestions multipleChoiceQuestions={selectedQuestions} examNumber={3} examType="lecture" />;
}
