...
export default async function LabExamFourQuestions() {
  ...
  // Fetch questions for lab exam 4
  const { data: allQuestions, error } = await supabase.from("exam_questions").select("*").eq("exam_type", "lab").eq("exam_number", "4");
  ...
  return <ExamQuestions multipleChoiceQuestions={selectedQuestions} examNumber={4} examType="lab" />;
}
