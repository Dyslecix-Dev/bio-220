...
export default async function LabExamThreeQuestions() {
  const supabase = await createClient();

  // Fetch questions for lab exam 3
  const { data: allQuestions, error } = await supabase.from("exam_questions").select("*").eq("exam_type", "lab").eq("exam_number", "3");
