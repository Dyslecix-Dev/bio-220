...
const EXAM_NUMBER_OPTIONS: Record<string, { value: string; label: string }[]> = {
  lecture: Array.from({ length: 16 }, (_, i) => ({
    value: String(i + 1),
    label: `Chapter ${i + 1}`,
  })),
...
export default function EditExamQuestion() {
  ...
  // Fetch exam question data on mount
  useEffect(() => {
    const fetchExamQuestion = async () => {
      try {
        const supabase = await createClient();

        const { data, error: fetchError } = await supabase.from("exam_questions").select("*").eq("id", questionId).single();

        if (fetchError) {
          console.error("Error fetching exam question:", fetchError);
          setError(`Failed to load exam question: ${fetchError.message}`);
          setLoading(false);
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user) {
        setError("You must be logged in to edit exam questions");
        setLoading(false);
        return;
      }
      ...
      if (updateError) {
        console.error("Error updating exam question:", updateError);
        setError(`Failed to update exam question: ${updateError.message}`);
        setLoading(false);
        return;
      }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-zinc-950 text-white">
      ...
      <section className="pt-32 px-6 max-w-3xl mx-auto pb-12">
        <h1 className="text-4xl font-bold mb-8">Edit Exam Question</h1>
