  ...
  return (
    <main className="min-h-screen w-full flex items-center justify-center bg-zinc-950 text-zinc-100">
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-16 md:grid-cols-2 md:gap-8">
        <div>
          <h3 className="text-5xl font-black leading-[1.25] md:text-7xl">BIO 220</h3>
...
const Card = ({ handleShuffle, position, imgUrl, imgAlt }: CardProps) => {
  ...
  return (
    <motion.div
      ...
      className={`absolute left-0 top-0 grid h-full w-full select-none place-content-center space-y-6 rounded-2xl border-2 border-slate-700 bg-slate-800/20 p-6 shadow-xl backdrop-blur-md ${
        draggable ? "cursor-grab active:cursor-grabbing" : ""
      }`}
    >
      <Image src={imgUrl} alt={imgAlt} height={100} width={100} className="pointer-events-none mx-auto h-full w-full object-cover" />
    </motion.div>
  );
};
